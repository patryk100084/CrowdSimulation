...
public class SMPLBlendshapes : MonoBehaviour {
	...
	void Start () 
	{
		...
		_modifyBones = new SMPLModifyBones(targetMeshRenderer, parent);
		...
		// 5. Set shape parameters (betas) of avg mesh in FBX model 
		// 	  to shape-parameters (betas) from user's JSON file
		setShapeBlendValues();

		// 6. Calculate joint-locations from betas & update joints of the FBX model
		calculateJoints();		
	...
	public void calculateJoints()
	{
		...
		_modifyBones.updateBonePositions(joints, true);
	}
}